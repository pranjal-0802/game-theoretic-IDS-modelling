...
class Model:
  ...
  def next_state(self, mode, state, delta, alpha, rnd=None):
    ...
    """
    f1 = open('true alerts_4_G.csv', 'a')
    writer1 = csv.writer(f1)
    f2 = open('true alerts_2_G.csv', 'a')
    writer2 = csv.writer(f2)
    f3 = open('true alerts_4_I.csv', 'a')
    writer3 = csv.writer(f3)
    f4 = open('true alerts_2_I.csv', 'a')
    writer4 = csv.writer(f4)
    f5 = open('benign_alerts_G.csv', 'a')
    writer5 = csv.writer(f5)
    f6 = open('benign_alerts_I.csv', 'a')
    ...
    for a in range(len(self.attack_types)):
          ...
          for t in range(len(self.alert_types)):
                b.append(next.R[0][a][t])
      
    #print("true alerts genereated is {}".format(b))
    writer1.writerow([b])
    c1=[]
    c1.insert(0,b[0]+b[2])
    c1.insert(1,b[1]+b[3])
    #print("true alerts generated for each alert type {}".format(c1))
    writer2.writerow([c1])
    

    for t in range(len(self.alert_types)):
      a=self.alert_types[t].false_alerts.generate()
      al.append(a)
      next.N[0][t] = a + sum((next.R[0][a][t] for a in range(len(self.attack_types))))
    writer5.writerow([al])
    dell=[]
    for sublist in delta:
       for element in sublist:
        dell.append(element)
    #print("number of alerts to investigate {}".format(dell))
    writer7.writerow([dell])
    rat=[]
    for i in range(len(self.alert_types)):
       rat.append(dell[i]/(al[i]+c1[i]))  
    #print("ratio is {}".format(rat)) 
    count1 =[]
    for i in range(len(self.alert_types)):
      count1.append(rat[i]*al[i])
    count=[]
    for i in count1:
       count.append(round(i))
    #print("number of benign alerts investigated {}".format(count))
    writer6.writerow([count])
    bb=b
   
    for q in range(len(bb)):
 
      if q == 0 or q==2 :
        bb[q]=round(rat[0]*bb[q])
      if q == 1 or q==3 :
        bb[q]=round(rat[1]*bb[q])
  
    #print("true alerts investigated_4 {} ".format(bb))
    writer3.writerow([bb])
                

    
    c11=[]
    c11.insert(0,bb[0]+bb[2])
    c11.insert(1,bb[1]+bb[3])
 
    #print("true alerts investigated_2 {}".format(c11))
    writer4.writerow([c11])
