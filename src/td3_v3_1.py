...
class DDPGbase(object):
    def __init__(self, a_dim, s_dim):
        ...
        with tf.control_dependencies(target_update):    # soft replacement happened at here
            q_min = tf.minimum(q1_, q2_)
            q_target = self.R + GAMMA * q_min
            ...
            self.ctrain1 = tf.train.AdamOptimizer(LR_C).minimize(td_error1, var_list=c1_params)
            self.ctrain2 = tf.train.AdamOptimizer(LR_C).minimize(td_error2, var_list=c2_params)
        self.sess.run(tf.global_variables_initializer())

    def choose_action(self, s):
        return self.sess.run(self.a, {self.S: s[np.newaxis, :]})[0]

    def Q_value1(self, s, a):
        return self.sess.run(self.Qvalue1, {self.S: s[np.newaxis, :], self.a: a[np.newaxis, :]})[0]

    def Q_value2(self, s, a):
        return self.sess.run(self.Qvalue2, {self.S: s[np.newaxis, :], self.a: a[np.newaxis, :]})[0]
    ...
    def learn_from_mix(self, model, initial_state, state_observe, state_update, op_profile, op_strategy):
        ...
        for i in range(MAX_EPISODES):
            ...
            for j in range(MAX_EP_STEPS):
                ...
                noise = np.random.normal(0, 1*epsilon, action.shape)
