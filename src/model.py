...
class AttackType:
  ...
  def __init__(self, loss, cost, pr_alert, name, gain=None):
    """
    ...
    self.loss = loss
    self.gain = gain if gain is not None else loss  # defaults to loss → zero-sum behaviour
    self.cost = cost
  ...
  class State:
    ...
    def __init__(self, model, N=None, M=None, R=None, U=None):
      ...
      if N is None:
        ...
        self.R = R
        self.U = U
        self.U_defender = U if U is not None else 0.0
        self.U_attacker = 0.0
    def __str__(self):
  ...
  def next_state(self, mode, state, delta, alpha, rnd=None):
    ...
    if mode == 'new':
      ...
      for a in range(len(self.attack_types)):
        ...
        atk_gain += self.attack_types[a].gain[0] * state.M[0][a]
    ...
    next.U_defender = state.U_defender + def_loss
    next.U_attacker = state.U_attacker + atk_gain - sum(
        self.attack_types[a].cost * pr_attacks[a]
        for a in range(len(self.attack_types))
    )
