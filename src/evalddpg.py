...
class DDPGlearning3:
    ...
    def __init__(self, mode, state_size, action_size):
        ...
        self.utility = 0.0
        if self.mode == "defend":
            self.ddpg = DDPGdefend3(action_size, state_size)
        else:
            self.ddpg = DDPGattack3(action_size, state_size)
...
class DefenderOracle3:
    ...
    def __init__(self, model_name, model, def_budget, estimate_adv_budget, exper_index, iteration_index):
        ...
        self.mode = "defend"
        self.agent = DDPGlearning3(self.mode, len(model.alert_types) * model.horizon, len(model.alert_types) * model.horizon)
...
class AttackerOracle3:
    ...
    def __init__(self, model, defense_profile, defense_strategy):
        ...
        self.agent = DDPGlearning3(self.mode, state_size, action_size)
        saver = tf.train.Saver()
